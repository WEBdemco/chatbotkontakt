...
export function loadGenders() {
  ...
  console.log(chatbotState.language);
  var genders = ["Femme", "Homme", "Fille" , "Garçon" , "Bébés", "Cancel"];
  if(chatbotState.language ==="anglais"){
    
    genders = ["Wompen", "men", "girl" , "Garçon" , "Bébés", "Cancel"];
  }
  
  genders.forEach((gender) => {
    createButton(gender, "gendersContainer", () => {
      // Display the user's selected gender
      if(chatbotState.language == "anglais"){ addChatUserMessage(`Genre: ${gender}`);}
      addChatUserMessage(` ${gender}`);
     
      
      // Update chatbot state with selected gender
      chatbotState.gender = gender;
      console.log("lang",chatbotState.language);
      // Show the loading message where the bot's response will be
      showLoadingMessage(true);
...
function loadPriceOptions() {
  ...
  const priceOptions = [ "sélectionnez votre prix"];
...
function resetChat() {
  ...
  <div id="start_conv">
    ...
    <span style="vertical-align: middle">Marhba bik fi KONTAKT 🥰! Inscrivez-vous dès maintenant pour rejoindre notre grande famille 💛👨‍👩‍👧‍👧<br><a href='https://kontakt.com.tn/web/login' target='_blank'>kontakt.com.tn.SignUp </a><br> kifech najmou naawnouk ?</span>
  </div>
